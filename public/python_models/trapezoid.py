"""Composite trapezoidal rule (Newton-Cotes, n=1).

Approximates ∫[a,b] f(x) dx by joining (xᵢ, f(xᵢ)) and (xᵢ₊₁, f(xᵢ₊₁))
with straight segments and summing the trapezoid areas:

    ∫[a,b] f(x) dx ≈ h/2 · [ f(a) + 2·Σᵢ f(xᵢ) + f(b) ],   h = (b−a)/n
"""
...
def solve(params: dict[str, Any]) -> dict[str, Any]:
    ...
    # Composite trapezoid: weights 1, 2, 2, ..., 2, 1
    integral = (h / 2.0) * (ys[0] + 2.0 * sum(ys[1:n]) + ys[-1])
    ...
    # Markers at the trapezoid vertices
